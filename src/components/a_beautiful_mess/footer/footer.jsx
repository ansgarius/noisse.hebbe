import React  from 'react';
import "./footer.css";

const Footer=()=>{

    return(
        <>
         <div className="a_beautiful_mess_footer footer-section"
            data-scroll-section="true" data-scroll-section-id="section11" data-scroll-section-inview>
                <div data-scroll="true" data-scroll-repeat="true" data-scroll-call="footer" className='rowFooter' >
                    
                    
                    <div className="wrapperTittle">
                        <h2 data-scroll="true" className="locomotive-fadeInUp  hover-underline  ">

                            YOUR STORY DESERVE TO BE TOLD
                       
                        </h2>
                    </div>


<div className='verticalFooterWrapper'>

                    <div className="wrapperContent">
                        <div className='itemCenter  columnLearMore'>
                            <h2 data-scroll="true" className="locomotive-fadeInUp   ">
                                LEARN MORE
                            </h2>

                            <div>
                            <h2 data-scroll="true" className="locomotive-fadeInUp   ">
                                F.A.Q
                            </h2>


                            <h2 data-scroll="true" className="locomotive-fadeInUp   ">
                                RESOURCES
                            </h2>
                            </div>
                            <div></div>
                        </div>

                        <div className='itemCenter'>
                        <img  data-v-4d3e24b1="" src="assets/img/hebbeLogo.svg" 
                                alt="Agencia de Branding y Publicidad Kids." className="header-logo show"/>

                            <div className='social'  >
            <ul >
              <li>  
                <svg  xmlns="http://www.w3.org/2000/svg"     viewBox="0 0 48 48"
                fill="none"  >
                <path width={24}
                fill="#CBCBCB"
                d="M47.522 14.4s-.469-3.31-1.913-4.762c-1.828-1.913-3.872-1.922-4.809-2.035-6.712-.487-16.79-.487-16.79-.487h-.02s-10.077 0-16.79.487c-.938.113-2.981.122-4.81 2.035C.948 11.09.488 14.4.488 14.4S0 18.29 0 22.172v3.637c0 3.882.478 7.772.478 7.772s.469 3.31 1.903 4.763c1.828 1.912 4.228 1.847 5.297 2.053 3.844.366 16.322.478 16.322.478s10.087-.019 16.8-.497c.938-.112 2.981-.122 4.81-2.034 1.443-1.453 1.912-4.763 1.912-4.763S48 29.701 48 25.81v-3.637c0-3.881-.478-7.772-.478-7.772ZM19.04 30.225v-13.49l12.965 6.768-12.965 6.722Z"
                />
                </svg>
              </li>
              <li>
              <svg viewBox="0 0 48 48"
    xmlns="http://www.w3.org/2000/svg"
    fill="none"
    
  >
    <path
      fill="#CBCBCB"
      d="M34.145 0h-8.09v32.696c0 3.895-3.11 7.095-6.982 7.095-3.872 0-6.984-3.2-6.984-7.095 0-3.826 3.043-6.957 6.776-7.096v-8.209C10.637 17.531 4 24.278 4 32.696 4 41.183 10.776 48 19.142 48s15.141-6.887 15.141-15.304V15.93A18.732 18.732 0 0 0 45 19.548v-8.209C38.916 11.13 34.145 6.122 34.145 0Z"
    />
  </svg>
                </li>

              <li>
              <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 48 48"
    fill="none"
     
  >
    <path
      fill="#CBCBCB"
      d="M35.384 22.247c-.206-.099-.416-.194-.63-.286-.37-6.826-4.1-10.735-10.363-10.775h-.085c-3.747 0-6.863 1.6-8.78 4.51l3.444 2.362c1.433-2.173 3.681-2.637 5.337-2.637h.057c2.063.014 3.619.613 4.626 1.782.733.852 1.224 2.028 1.466 3.513-1.828-.31-3.806-.406-5.92-.285-5.955.343-9.783 3.816-9.526 8.642.13 2.448 1.35 4.554 3.434 5.93 1.762 1.163 4.03 1.731 6.39 1.603 3.114-.171 5.557-1.36 7.261-3.532 1.295-1.65 2.114-3.788 2.475-6.482 1.484.895 2.584 2.074 3.192 3.491 1.033 2.409 1.093 6.367-2.137 9.594-2.83 2.827-6.231 4.05-11.372 4.088-5.702-.043-10.015-1.871-12.819-5.436C8.81 34.992 7.452 30.17 7.401 24c.05-6.17 1.408-10.992 4.034-14.33 2.803-3.564 7.116-5.393 12.818-5.435 5.744.043 10.132 1.88 13.043 5.462 1.428 1.756 2.504 3.965 3.213 6.54l4.037-1.076c-.86-3.17-2.213-5.902-4.055-8.168C36.76 2.402 31.301.05 24.267 0h-.028C17.22.049 11.823 2.41 8.197 7.02c-3.227 4.102-4.89 9.809-4.947 16.963v.034c.056 7.154 1.72 12.862 4.947 16.963 3.626 4.61 9.023 6.972 16.042 7.02h.028c6.24-.043 10.639-1.677 14.262-5.297 4.741-4.736 4.598-10.673 3.036-14.318-1.121-2.613-3.258-4.736-6.18-6.138ZM24.61 32.377c-2.61.147-5.321-1.025-5.455-3.534-.1-1.86 1.324-3.936 5.615-4.184a25.08 25.08 0 0 1 1.448-.042c1.558 0 3.017.151 4.343.441-.495 6.176-3.395 7.178-5.95 7.319Z"
    />
  </svg>
                </li>
       

              <li>
              <svg
    xmlns="http://www.w3.org/2000/svg"
viewBox="0 0 48 48"
    fill="none"
   
  >
    <g fill="#CBCBCB" clipPath="url(#a)">
      <path d="M24 4.322c6.413 0 7.172.028 9.694.14 2.343.104 3.61.497 4.453.825 1.116.432 1.922.957 2.756 1.791.844.844 1.36 1.64 1.79 2.756.329.844.723 2.12.826 4.454.112 2.53.14 3.29.14 9.693 0 6.413-.028 7.172-.14 9.694-.103 2.344-.497 3.61-.825 4.453-.431 1.116-.957 1.922-1.79 2.756-.845.844-1.642 1.36-2.757 1.791-.844.328-2.119.722-4.453.825-2.532.112-3.29.14-9.694.14-6.413 0-7.172-.028-9.694-.14-2.343-.103-3.61-.497-4.453-.825-1.115-.431-1.922-.956-2.756-1.79-.844-.844-1.36-1.641-1.79-2.757-.329-.844-.723-2.119-.826-4.453-.112-2.531-.14-3.29-.14-9.694 0-6.412.028-7.172.14-9.694.103-2.343.497-3.609.825-4.453.431-1.115.957-1.921 1.79-2.756.845-.844 1.642-1.36 2.757-1.79.844-.329 2.119-.722 4.453-.825 2.522-.113 3.281-.141 9.694-.141ZM24 0c-6.516 0-7.331.028-9.89.14-2.55.113-4.304.526-5.822 1.116-1.585.619-2.926 1.435-4.257 2.775-1.34 1.332-2.156 2.672-2.775 4.247C.666 9.806.253 11.55.141 14.1.028 16.669 0 17.484 0 24s.028 7.331.14 9.89c.113 2.55.526 4.304 1.116 5.822.619 1.585 1.435 2.925 2.775 4.257a11.732 11.732 0 0 0 4.247 2.765c1.528.591 3.272 1.003 5.822 1.116 2.56.112 3.375.14 9.89.14 6.516 0 7.332-.028 9.891-.14 2.55-.113 4.303-.525 5.822-1.116a11.732 11.732 0 0 0 4.247-2.765 11.732 11.732 0 0 0 2.766-4.247c.59-1.528 1.003-3.272 1.115-5.822.113-2.56.14-3.375.14-9.89 0-6.516-.027-7.332-.14-9.891-.112-2.55-.525-4.303-1.115-5.822-.591-1.594-1.407-2.935-2.747-4.266a11.732 11.732 0 0 0-4.247-2.765C38.194.675 36.45.262 33.9.15 31.331.028 30.516 0 24 0Z" />
      <path d="M24 11.672c-6.806 0-12.328 5.522-12.328 12.328 0 6.806 5.522 12.328 12.328 12.328 6.806 0 12.328-5.522 12.328-12.328 0-6.806-5.522-12.328-12.328-12.328Zm0 20.325a7.998 7.998 0 0 1 0-15.994 7.998 7.998 0 0 1 0 15.994ZM39.694 11.184a2.879 2.879 0 1 1-2.878-2.878 2.885 2.885 0 0 1 2.878 2.878Z" />
    </g>
    <defs>
      <clipPath id="a">
        <path fill="#fff" d="M0 0h48v48H0z" />
      </clipPath>
    </defs>
  </svg>
               
               </li>
                     
              <li>
                <svg  viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
<g clipPath="url(#clip0_17_61)">
<path d="M48 24C48 10.7452 37.2548 0 24 0C10.7452 0 0 10.7452 0 24C0 35.9789 8.77641 45.908 20.25 47.7084V30.9375H14.1562V24H20.25V18.7125C20.25 12.6975 23.8331 9.375 29.3152 9.375C31.9402 9.375 34.6875 9.84375 34.6875 9.84375V15.75H31.6613C28.68 15.75 27.75 17.6002 27.75 19.5V24H34.4062L33.3422 30.9375H27.75V47.7084C39.2236 45.908 48 35.9789 48 24Z" fill="#CBCBCB"/>
</g>
<defs>
<clipPath id="clip0_17_61">
<rect width="48" height="48" fill="white"/>
</clipPath>
</defs>
</svg>
</li>  
            </ul>
          </div>






          <div className='hidden' style={{textAlign:'center'}}>

          <h2 data-scroll="true" className="locomotive-fadeInUp   ">
                                LEARN MORE
                            </h2>
          </div>

                            <div className='hidden'>
                            <h2 data-scroll="true" className="locomotive-fadeInUp   ">
                                F.A.Q
                            </h2>


                            <h2 data-scroll="true" className="locomotive-fadeInUp   ">
                                RESOURCES
                            </h2>

                            <h2 data-scroll="true" className="locomotive-fadeInUp   ">
                                 NEWSLETTER
                            </h2>
                            </div>


                            

                            <div className='wrapperPolicy' >

                            <h2 data-scroll="true" className="locomotive-fadeInUp   ">
                                TERMS CONDITIONS
                            </h2>

                            <h2 data-scroll="true" className="locomotive-fadeInUp   ">
                                PRIVACY POLICE
                            </h2>

                            <h2 data-scroll="true" className="locomotive-fadeInUp   ">
                                MAP SITE
                            </h2>
                            </div>
                        </div>


                        <div className='itemCenter columMapSite'>
                            <h2 data-scroll="true" className="locomotive-fadeInUp   ">
                                 NEWSLETTER
                            </h2>


                            <div className='wrapperSiteMap'>
                       


































                            <div className='social' >
           
           
                <svg  xmlns="http://www.w3.org/2000/svg"     viewBox="0 0 48 48"
                fill="none"  >
                <path width={24}
                fill="#CBCBCB"
                d="M47.522 14.4s-.469-3.31-1.913-4.762c-1.828-1.913-3.872-1.922-4.809-2.035-6.712-.487-16.79-.487-16.79-.487h-.02s-10.077 0-16.79.487c-.938.113-2.981.122-4.81 2.035C.948 11.09.488 14.4.488 14.4S0 18.29 0 22.172v3.637c0 3.882.478 7.772.478 7.772s.469 3.31 1.903 4.763c1.828 1.912 4.228 1.847 5.297 2.053 3.844.366 16.322.478 16.322.478s10.087-.019 16.8-.497c.938-.112 2.981-.122 4.81-2.034 1.443-1.453 1.912-4.763 1.912-4.763S48 29.701 48 25.81v-3.637c0-3.881-.478-7.772-.478-7.772ZM19.04 30.225v-13.49l12.965 6.768-12.965 6.722Z"
                />
                </svg>
             
        
 
       

            
          </div>






          <div className='social' >
           
           
    
        
         <svg viewBox="0 0 48 48"
xmlns="http://www.w3.org/2000/svg"
fill="none"

>
<path
 fill="#CBCBCB"
 d="M34.145 0h-8.09v32.696c0 3.895-3.11 7.095-6.982 7.095-3.872 0-6.984-3.2-6.984-7.095 0-3.826 3.043-6.957 6.776-7.096v-8.209C10.637 17.531 4 24.278 4 32.696 4 41.183 10.776 48 19.142 48s15.141-6.887 15.141-15.304V15.93A18.732 18.732 0 0 0 45 19.548v-8.209C38.916 11.13 34.145 6.122 34.145 0Z"
/>
</svg> 

  

       
     </div>

























                            </div>

                            <div></div>
                        </div>
                    </div>

{/*                     <div className="wrapperContent">

                        <div className='itemEnd'>
                        <h2 data-scroll="true" className="locomotive-fadeInUp   ">
                            F.A.Q
                        </h2>
                        </div>
                        <div className='itemCenter'>
                        <h2 data-scroll="true" className="locomotive-fadeInUp   ">
                            LEARN MORE
                        </h2>
                        </div>
                        <div className='itemEnd'>
                        <h2 data-scroll="true" className="locomotive-fadeInUp   ">
                            LEARN MORE
                        </h2>
                        </div>
                       
                        
                       
                    </div>


                    <div className="wrapperContent">


                        <div className='itemCenter'>
                        <h2 data-scroll="true" className="locomotive-fadeInUp   ">
                            RESOURCES
                        </h2>
                        </div>
                        <div className='itemEnd'>
                        <h2 data-scroll="true" className="locomotive-fadeInUp   ">
                            LEARN MORE
                        </h2>
                        </div>
                        <div className='itemCenter'>
                        <h2 data-scroll="true" className="locomotive-fadeInUp   ">
                            LEARN MORE
                        </h2>
                        </div>
                       
                        
                        
                    </div> */}
</div>


                    <div className="wrapperTittle">
                        <div className='itemEnd'>
                        <h2 data-scroll="true" className="locomotive-fadeInUp  hover-underline  ">

YOUR LOVE STORY  

</h2>
                        </div>
                    </div>



              




{/* 
    <div className="col-lg-7 offset-xl-1"><ul data-scroll="true" className="credit-list  ">
        <li className="credit-item u-label" style={{transitionDelay: "0.15s"}}>
            <a href="https://senimanruang.com/" target="_blank">
                <span><div className="row">
                    <div className="col-md-4 col-name">Seniman Ruang</div>
                <div className="col-md-8 col-as">Architect &amp; Installation Designer</div></div></span>
            </a>
        </li>
        
 
 
 
    </ul></div>


 */}















   {/*      <div data-scroll="true"className="col-12 footer-abm locomotive-fadeInUp delayp3 ">
            <div data-gatsby-image-wrapper=""className="gatsby-image-wrapper gatsby-image-wrapper-constrained">
               
                        
              
                        <picture>
                            <source type="image/webp" srcSet="assets/img/img-logo-footer.webp 681w,
assets/img/img-logo-footer.webp 1362w,
assets/img/img-logo-footer.webp 2724w" sizes="(min-width: 2724px) 2724px, 100vw"/>
<img data-main-image="" style={{objectFit: "cover", opacity: "1"}} sizes="(min-width: 2724px) 2724px, 100vw" decoding="async" loading="lazy"
 src="assets/img/img-logo-footer.png" srcSet="assets/img/img-logo-footer.png 681w,
assets/img/img-logo-footer.png 1362w,
assets/img/img-logo-footer.png 2724w" alt="img-footer"/></picture>






</div></div> */}


</div>
            </div> 
        </>
    );
}

export default Footer;